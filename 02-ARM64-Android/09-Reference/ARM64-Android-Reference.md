...
Quick-consultation material for **ARM64-Android**: glossary, bibliography, plus a topic-wide, always-up-to-date overview of every reference entry, worked example, and real-world snippet — regardless of which chapter folder they live in.
...
FROM "02-ARM64-Android" AND #reference-entry
SORT file.name ASC
```
...
FROM "02-ARM64-Android" AND #example
...
FROM "02-ARM64-Android" AND #snippet
...
For a full project-by-project investigation (imported source kept in its real file/module layout, plus open-ended analysis notes not scoped to a single chapter), see [[Case-Studies]] rather than the per-chapter `snippets/` above.
