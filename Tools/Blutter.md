...
Extracts a Flutter app's Dart AOT snapshot from `libapp.so` and disassembles it to ARM64, annotated with class/field/function names recovered from the snapshot's own metadata (object pool contents, class table, function symbols where present). This is the tool behind every blutter-annotated real-world excerpt in this topic's chapters — without it, `libapp.so` is just a stripped ELF full of ARM64 with no names attached at all.
