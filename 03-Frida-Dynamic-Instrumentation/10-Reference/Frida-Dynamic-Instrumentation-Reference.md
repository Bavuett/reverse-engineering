...
FROM "03-Frida-Dynamic-Instrumentation" AND #reference-entry
SORT file.name ASC
```
...
FROM "03-Frida-Dynamic-Instrumentation" AND #example
...
FROM "03-Frida-Dynamic-Instrumentation" AND #snippet
...
[[Case-Studies]] rather than the per-chapter `snippets/` above.
