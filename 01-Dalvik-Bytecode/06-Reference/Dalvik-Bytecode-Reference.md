...
FROM "01-Dalvik-Bytecode" AND #reference-entry
SORT file.name ASC
```
...
FROM "01-Dalvik-Bytecode" AND #example
...
FROM "01-Dalvik-Bytecode" AND #snippet
...
For a full app-by-app investigation (imported source kept in its real class/package layout, plus open-ended analysis notes not scoped to a single chapter), see [[Case-Studies]] rather than the per-chapter `snippets/` above.
