...
- [blutter](https://github.com/worawit/blutter) — extracts and disassembles Dart AOT snapshots from Flutter `libapp.so`, annotating raw ARM64 with class/field/function names recovered from the snapshot metadata. The `.dart`-named files under this topic's [[Classeviva-Flutter-Case-Study|Classeviva case study]] are blutter output, not real Dart source.
