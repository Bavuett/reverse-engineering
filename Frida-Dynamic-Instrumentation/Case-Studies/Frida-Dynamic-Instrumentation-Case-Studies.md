---
tags: [moc]
aliases: ["Frida Dynamic Instrumentation Case Studies"]
---

# Frida-Dynamic-Instrumentation · Case Studies
...
FROM "Frida-Dynamic-Instrumentation" AND #case-study
...
FROM "Frida-Dynamic-Instrumentation" AND #case-study-note
SORT created DESC
```
...
1. Create `Frida-Dynamic-Instrumentation/Case-Studies/<project-name>/` and use the [[New-Case-Study]] template for
   its overview note.
2. Import only the files you actually need into
   `Frida-Dynamic-Instrumentation/Case-Studies/<project-name>/source/`, keeping their original file/module layout.
3. Add analysis notes under `Frida-Dynamic-Instrumentation/Case-Studies/<project-name>/notes/` using the
   [[New-Case-Study-Note]] template.
4. Add a `folder_templates` entry mapping `Frida-Dynamic-Instrumentation/Case-Studies/<project-name>/notes` →
